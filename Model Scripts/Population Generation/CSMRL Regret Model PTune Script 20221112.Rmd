...
c<-c(.2,seq(.5,2,length.out=4))
cRecip<-c(.2,seq(.5,2,length.out=4))



#Thompson SD#
#Starting SD for Thompson sampling and Thompson Reciprocal
tSD<-seq(.1,.3,length.out=5)
tSDRecip<-seq(.1,.3,length.out=5)



#Epsilon#
#Probability of exploration for the Epsilon Greegy 
#and Epsilon Reciprocal investment strategy
e<-c(.01,seq(.05,.20,length.out=4))
eRecip<-c(.01,seq(.05,.20,length.out=4))
...
#Extract mated PINs for females
fRels<-data.frame("PIN"=females$PIN)
fRels$mPIN<-fullPairs[match(females$PIN,fullPairs[,34]),68]

#Do the same for males
mRels<-data.frame("PIN"=males$PIN)
mRels$mPIN<-fullPairs[match(males$PIN,fullPairs[,68]),34]

#Rbind together
fullRels<-rbind(fRels,mRels)

#Set mated PINs of single agents to 0
fullRels$mPIN[is.na(fullRels$mPIN)]<-0
...
data<-data.frame("s"=NA,"loop"=NA,"model"=NA,"obs"=NA,"acc"=NA,"r"=NA)
data<-data[-1,]


#Loop through parameter setting loops
...
for(s in 1:5){
  ...
  for(l in 1:loops){
    ...
    thomSDMale<-matrix(tSD[s],nrow(males),nrow(females))
    thomSDFemale<-matrix(tSD[s],nrow(females),nrow(males))
    ...
    tRecipSDMale<-matrix(tSDRecip[s],nrow(males),nrow(females))
    tRecipSDFemale<-matrix(tSDRecip[s],nrow(females),nrow(males))
    ...
    for(b in 1:blocks){
      ...
      for(o in 1:1000){
        ...
        cm<-c[s]*sqrt(log(rowSums(ucbInvestMale)[1])/ucbInvestMale)
        cf<-c[s]*sqrt(log(rowSums(ucbInvestFemale)[1])/ucbInvestFemale)
        ...
        #Calculate the constant that will be added to MV values
        #Increases the less frequently a potential mate is observed
        cm<-cRecip[s]*sqrt(log(rowSums(uRecipInvestMale)[1])/uRecipInvestMale)
        cf<-cRecip[s]*sqrt(log(rowSums(uRecipInvestFemale)[1])/uRecipInvestFemale)
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-epsilPairs[match(females$PIN,epsilPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-epsilPairs[match(males$PIN,epsilPairs[,68]),34]
      
      #Rbind together
      epsilBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      epsilBlockRels$mPIN[is.na(epsilBlockRels$mPIN)]<-0
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-ucbPairs[match(females$PIN,ucbPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-ucbPairs[match(males$PIN,ucbPairs[,68]),34]
      
      #Rbind together
      ucbBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      ucbBlockRels$mPIN[is.na(ucbBlockRels$mPIN)]<-0
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-thomPairs[match(females$PIN,thomPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-thomPairs[match(males$PIN,thomPairs[,68]),34]
      
      #Rbind together
      thomBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      thomBlockRels$mPIN[is.na(thomBlockRels$mPIN)]<-0
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-eRecipPairs[match(females$PIN,eRecipPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-eRecipPairs[match(males$PIN,eRecipPairs[,68]),34]
      
      #Rbind together
      eRecipBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      eRecipBlockRels$mPIN[is.na(eRecipBlockRels$mPIN)]<-0
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-uRecipPairs[match(females$PIN,uRecipPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-uRecipPairs[match(males$PIN,uRecipPairs[,68]),34]
      
      #Rbind together
      uRecipBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      uRecipBlockRels$mPIN[is.na(uRecipBlockRels$mPIN)]<-0
      ...
      #Extract mated PINs for females
      fRels<-data.frame("PIN"=females$PIN)
      fRels$mPIN<-tRecipPairs[match(females$PIN,tRecipPairs[,34]),68]
      
      #Do the same for males
      mRels<-data.frame("PIN"=males$PIN)
      mRels$mPIN<-tRecipPairs[match(males$PIN,tRecipPairs[,68]),34]
      
      #Rbind together
      tRecipBlockRels<-rbind(fRels,mRels)
      
      #Set mated PINs of single agents to 0
      tRecipBlockRels$mPIN[is.na(tRecipBlockRels$mPIN)]<-0
    ...
    epsilRels$acc<-epsilRels$mPIN==fullRels$mPIN[
      match(epsilRels$PIN,fullRels$PIN)]
    
    ucbRels$acc<-ucbRels$mPIN==fullRels$mPIN[
      match(ucbRels$PIN,fullRels$PIN)]
    
    thomRels$acc<-thomRels$mPIN==fullRels$mPIN[
      match(thomRels$PIN,fullRels$PIN)]
    
    eRecipRels$acc<-eRecipRels$mPIN==fullRels$mPIN[
      match(eRecipRels$PIN,fullRels$PIN)]
    
    uRecipRels$acc<-uRecipRels$mPIN==fullRels$mPIN[
      match(uRecipRels$PIN,fullRels$PIN)]
    
    tRecipRels$acc<-tRecipRels$mPIN==fullRels$mPIN[
      match(tRecipRels$PIN,fullRels$PIN)]
    ...
    relData<-rbind(epsilRels,thomRels,
                   ucbRels,eRecipRels,tRecipRels,uRecipRels)
    
    #Create a dataframe for summarized results
    loopData<-data.frame("s"=s,"loop"=l,"model"=rep(sort(unique(relData$model)),
...
#Create a dataframe for summarized results
data2<-data.frame("param"=rep(1:5,each=length(unique(data$obs))*
                                length(unique(data$model))),
                  "model"=rep(sort(unique(data$model)),
                              each=length(unique(data$obs)),times=5),
                 "obs"=rep(unique(data$obs),
                           length(unique(data$model))*5))

#Compute average accuracy across parameter settings, models, and observation blocks
data2$acc<-c(sapply(1:5,function(y) tapply(data$acc[data$s==y],
                    list(data$obs[data$s==y],data$model[data$s==y]),function(x)
  mean(x))))


#Reorder model levels
data2$model<-factor(data2$model,
                    levels=c("Epsilon Greedy",
                             "Thompson\nSampling",
                             "UCB",
                             "Epsilon\nReciprocity",
                             "Thompson\nReciprocity",
...
plotData<-melt(data2,id.vars=c("model","obs","param"))



###Epsilon Plots###

#Create a dataframe for plotting just Epsilon models
ePlotData<-plotData[plotData$model %in% c("Epsilon Greedy",
                                                      "Epsilon\nReciprocity"),]

#Relabel parameter settings appropriately
ePlotData$param<-e[ePlotData$param]

#Plot average simulation accuracy across models, observations, and parameter settings
eRegretPlot<-ggplot(data=ePlotData,
                    aes(x=obs,
                        y=value,
                        color=as.factor(param),
                        group=as.factor(param)))+
  labs(x="Number of Observations",
       y="Simulation Accuracy")+
  geom_line()+
  theme_grey(base_size=20)+
  scale_color_discrete(name="Parameter Setting")+
...
#Create a dataframe for plotting just Epsilon models
tPlotData<-plotData[plotData$model %in% c("Thompson\nSampling",
                                                      "Thompson\nReciprocity"),]

#Relabel parameter settings appropriately
tPlotData$param<-tSD[tPlotData$param]

#Plot average simulation accuracy across models, observations, and parameter settings
tRegretPlot<-ggplot(data=tPlotData,
                    aes(x=obs,
                        y=value,
                        color=as.factor(param),
                        group=as.factor(param)))+
...
#Create a dataframe for plotting just Epsilon models
uPlotData<-plotData[plotData$model %in% c("UCB","UCB\nReciprocity"),]

#Relabel parameter settings appropriately
uPlotData$param<-c[uPlotData$param]

#Plot average simulation accuracy across models, observations, and parameter settings
uRegretPlot<-ggplot(data=uPlotData,
                    aes(x=obs,
                        y=value,
                        color=as.factor(param),
                        group=as.factor(param)))+
...
ggsave("Figures/UCB Regret Parameter Tuning.tiff",plot=uRegretPlot,dpi=300,width=10,height=6,units="in")
ggsave("Figures/Epsilon Regret Parameter Tuning.tiff",plot=eRegretPlot,dpi=300,width=10,height=6,units="in")
ggsave("Figures/Thompson Regret Parameter Tuning.tiff",plot=tRegretPlot,dpi=300,width=10,height=6,units="in")
