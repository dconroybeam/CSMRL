...
for(l in 1:loops){
  ...
  for(b in 1:blocks){
    ...
    #Extract mated PINs for females
    fRels<-data.frame("PIN"=females$PIN)
    fRels$mPIN<-nlPairs[match(females$PIN,nlPairs[,34]),68]
    
    #Do the same for males
    mRels<-data.frame("PIN"=males$PIN)
    mRels$mPIN<-nlPairs[match(males$PIN,nlPairs[,68]),34]
    
    #Rbind together
    nlBlockRels<-rbind(fRels,mRels)
    
    #Set mated PINs of single agents to 0
    nlBlockRels$mPIN[is.na(nlBlockRels$mPIN)]<-0
  ...
  nlRels$acc<-nlRels$mPIN==fullRels$mPIN[
    match(nlRels$PIN,fullRels$PIN)]
...
regretPlot <- ggplot(data = data2,
  ...
  scale_x_continuous(breaks = seq(2000, 10000, 2000))
