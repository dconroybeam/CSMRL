...
facetLabs<-c("Simulation Accuracy (%)",
             "Assortative Mating (r)")

names(facetLabs)<-c("acc","r")

#Create a variable for reference lines
plotData$hline<-rep(c(1,fullAssort),each=nrow(plotData)/2)

#Plot average assortative mating and simulation accuracy across models and observations
regretPlot <- ggplot(data = plotData,
                     aes(x = obs,
                         y = value,
                         color = model)) +
  ...
  facet_wrap( ~ variable, ncol = 2,
              labeller = labeller(variable = facetLabs)) +
